...
export default function CustomerView({ branches, menu }: CustomerViewProps) {
  ...
  // Load URL params and apply branch theme
  useEffect(() => {
    ...
    if (branch && table && area) {
      ...
      // FIXED: Apply branch theme to document root
      ...
      // FIXED: Apply default theme
      document.documentElement.setAttribute('data-branch', defaultBranch);
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      // Enable real-time recognition with interim results for conversational turn-taking
      // FIXED: Set continuous to false to prevent continuous sound loops
  ...
  const toggleListening = () => {
    ...
    if (isMicSessionActive) {
      ...
      setVoiceStatus('idle');
      setIsListening(false);
      // FIXED: Removed mic_stop sound to reduce continuous beeping
    } else {
      ...
      try {
        ...
        // FIXED: Removed mic_start sound to reduce continuous beeping
      } catch (err) {
